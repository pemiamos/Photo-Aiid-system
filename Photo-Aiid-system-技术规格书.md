...
> **AI 照片管理 + 网页征稿协作系统**
> 版本：v3.3.0 · 桌面端本地整理 + 云端网页征稿（双版本模式）
> 日期：2026-06-30
...
- **v3.1 — 整理体验强化**：多文件夹持久库 + 历史索引（跨全库找图），原始文件名保留，HEIC/大图兼容，AI 二级类别「主类·子类」，提示词抽共享模块。✅
- **v3.2 — 照片归集闭环**：征稿看板多选存入相册，相册增删改名 + 整包 ZIP 引用式下载，按书停止/恢复征稿，每本书独立投稿码前缀，看板缩略图改用 OSS 图片处理提速。✅
- **v3.3 — 双版本模式与体验打磨**：普通用户版 / 高级摄影师版一键切换（默认普通版隐藏投稿/征稿，状态持久化）；征稿看板缩略图服务器本地缓存 + EXIF 转正 + 完整不裁切 + HEIC 转码；新增 OpenAI 识别引擎（实时拉取模型列表）；画廊/历史索引显示图片路径 + 打开所在文件夹、尺寸大小常显、历史索引点图放大；投稿管理记住上次书目。✅ （当前 v3.3.0）

**后续展望（未实现）**：本地人脸聚类、OCR 入索引、真·向量检索（sqlite-vec 替代 LLM 语义匹配）、查重去重、AI 批量修图。

---

**文档说明：** 本规格书由 pemiamos 与 AI（Claude / Codex / AG 三方接力）协作维护，描述截至 v3.3.0 **已实现并发布**的系统。各版本详细变更见 `docs/release-v*.md`。
